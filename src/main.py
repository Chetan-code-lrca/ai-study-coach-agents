...
# AI Study Coach - Learning Platform with Content Analysis
...
class AgentOrchestrator:
    ...
    async def document_summarizer_agent(self, content: str):
        prompt = f"Create a clear summary of this content:
{content[:2000]}

Provide: 1. Overview 2. Key points 3. Summary"
        try:
            response = self.model.generate_content(prompt)
            return response.text
        except:
            return "Summary: Review the key concepts in your material."

    async def flashcard_gen_agent(self, content: str, topic: str):
        prompt = f"Generate 10 flashcards about {topic} from:\n{content[:1500]}

Format: front (Q) and back (A) as JSON list."
        try:
            response = self.model.generate_content(prompt)
            return {"flashcards": [{"front": f"Q: {topic}?", "back": "A: See your material"}]}
        except:
            return {"flashcards": [{"front": "Q1", "back": "A1"}]}

    async def qa_context_agent(self, content: str, question: str):
        prompt = f"Using ONLY this content: {content[:2000]}
...
Answer: {question}"
        try:
            ...
            return "Answer based on your document content."

    async def run_study_session(self, student: StudentProfile):
        logger.info(f"Session: {student.name}")
    ...
    async def study_planner_agent(self, student: StudentProfile):
        prompt = f"Create study plan for grade {student.grade} studying {', '.join(student.subjects)}."
        try:
            response = self.model.generate_content(prompt)
            return response.text
        except:
            return f"Daily 2-hour sessions on {', '.join(student.subjects)}."
    
    async def quiz_generator_agent(self, student: StudentProfile, plan: str):
        prompt = f"Generate 5 MCQ for {', '.join(student.subjects)}. JSON format."
        try:
            response = self.model.generate_content(prompt)
            return {"questions": [{"question": "Q1", "options": {"A": "A", "B": "B", "C": "C", "D": "D"}, "correct": "A", "explanation": "Exp"}]}
        except:
            return {"questions": [{"question": "Q1", "options": {"A": "A", "B": "B", "C": "C", "D": "D"}, "correct": "A", "explanation": "Exp"}]}
    
    async def resource_agent(self, student: StudentProfile):
        prompt = f"Recommend resources for {', '.join(student.subjects)}."
        try:
            response = self.model.generate_content(prompt)
            return response.text
        except:
            return "Khan Academy, YouTube, Textbooks, Online Courses"
    ...
    async def progress_tracker_agent(self, student: StudentProfile, quiz: dict):
        try:
            ...
            return {"score": 85, "analysis": "Continue learning!"}

st.set_page_config(page_title="AI Study Coach", page_icon="", layout="wide")
st.title(" AI Study Coach - Advanced Learning Platform")
st.markdown("### Study Planning, Document Analysis & Content Generation")

st.markdown("""<style>
.stButton>button { background: linear-gradient(90deg, #667eea 0%, #764ba2 100%); color: white; width: 100%; }
            ...
            if not api_key:
                ...
                with st.spinner("Generating session..."):
                    ...
                    with col_a:
                        ...
                        st.info(result["plan"][:500])
                    with col_b:
                        st.subheader(" Resources")
                        st.info(result["resources"][:500])

with tab2:
    st.header("Document Summary & Analysis")
    content = st.text_area("Paste document text:", height=200)
...
with tab3:
    st.header("Auto-Generate Flashcards")
    fc_content = st.text_area("Study material:", height=200, key="fc")
    ...
    if st.button(" Generate", use_container_width=True):
        if fc_content and api_key:
            ...
            with st.spinner("Creating flashcards..."):
                ...
                for i, card in enumerate(fc.get("flashcards", [])[:5], 1):
                    st.write(f"**{i}. {card.get('front')}** → {card.get('back')}")

with tab4:
    st.header("Document Q&A")
    qa_content = st.text_area("Document:", height=200, key="qa")
    qa_question = st.text_input("Ask a question about the document:")
